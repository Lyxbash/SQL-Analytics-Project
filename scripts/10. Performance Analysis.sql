/*
 ...
 comparing each product's sales to both its average sales
...
product_name, 
current_sales,
AVG(current_sales) OVER (PARTITION BY product_name) product_avg_sales,
...
CASE 
	WHEN 
		current_sales - LAG(current_sales) OVER (PARTITION BY product_name ORDER BY order_year) > 0 THEN 'Increase (vs prev Yr)'
	WHEN	
		current_sales - LAG(current_sales) OVER (PARTITION BY product_name ORDER BY order_year) < 0 THEN 'Decrease (vs prev Yr)'
	ELSE
		'No change (flat)'
END vs_prev_year
